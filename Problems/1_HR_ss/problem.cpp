...
// Local Mode (for local tests, take input from files), or comment this to be in
// Submission mode (take input from CLI) #define ForceLOCAL
...
bool isLocalMode() {
  ...
  if (std::filesystem::is_regular_file(envFilePath)) {
    // Env var file exists
    std::ifstream file{

        std::ifstream(envFilePath, std::ios::in | std::ios::binary)};
